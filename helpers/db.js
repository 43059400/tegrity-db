...
module.exports = {
    ...
    upatePriorty: (user, item, alias, cb, pos) => {
        pool.getConnection((error, connection) => {
            connection.query(`SELECT * FROM wish_list_items INNER JOIN items on items.id = wish_list_items.item_id INNER JOIN users on users.id = wish_list_items.user_id WHERE users.id = '${user.id}' AND wish_list_items.alias_id = '${(alias.alias_id).toString()}' AND zone_id = '${item.zone_id}'`, (error, result) => {
                result = result || []
                let newPriorityList = []
                ...
                connection.query(`INSERT INTO wish_list_items_tracking(action_id, user_id, alias_id, item_id, details, time_stamp) VALUES ('3', '${user.id}', '${(alias.alias_id).toString()}', '${item.id}', '${JSON.stringify(item)}', now())`, (error, result) => {
                    error ? console.log(error) : console.log(`Create new wish_list_items_tracking ${user.id} ${(alias.alias_id).toString()} ${item.id} `)
                })
                ...
                newPriorityList.forEach((element) => {
                    connection.query(`UPDATE wish_list_items SET priority='${element.priority}', time_stamp=Now() WHERE user_id='${user.id}' AND alias_id='${(alias.alias_id).toString()}' AND item_id='${String(element.item_id)}'`, (error, result) => {
                        if (error) 
                        { console.log(error) }
                    })
                })
    ...
    insertWish: (user, item, alias, cb) => {
        pool.getConnection((error, connection) => { 
            let query = `SELECT * FROM wish_list_items INNER JOIN items on items.id = wish_list_items.item_id WHERE user_id='${user.id}' AND alias_id=${(alias.alias_id).toString()} AND zone_id='${item.zone_id}'`
            connection.query(query, (error, result) => {
                if(result.length >= 5) {
                    console.log(`5+ wishes already for ${user.id} ${(alias.alias_id).toString()} ${item.zone_id}`)
                    ...
                    if (result !== undefined) {
                        query = `INSERT INTO wish_list_items(user_id, alias_id, item_id, priority, time_stamp) VALUES (${user.id},${(alias.alias_id).toString()},${item.id}, 1, NOW())`
                        connection.query(query, (err, result) => {
                            err ? console.log(err) : console.log(`inserted wish ${user.id} ${(alias.alias_id).toString()} ${item.id}`)
                        })
                        ...
                        connection.query(query, (err, result) => {
                            err ? console.log(err) : console.log(`inserted wish into tracking ${user.id},${(alias.alias_id).toString()}, ${item.id}`)
                        })
                    } else {
                        cb([])
                    }
    ...
    deleteWish: (user, item, alias, cb) => {
        pool.getConnection((error, connection) => {     
            let query = `SELECT * FROM wish_list_items WHERE user_id=${user.id} AND alias_id=${(alias.alias_id).toString()} AND item_id="${item.id}"`
            connection.query(query, (err, row) => {
                if(row !== undefined) {
                    query = `DELETE FROM wish_list_items WHERE user_id=${user.id} AND alias_id=${(alias.alias_id).toString()} AND item_id=${item.id}`
                    connection.query(query, (err, result) => {
                        err ? console.log(err) : console.log(`Removed item ${user.id} ${(alias.alias_id).toString() } : ${item.id}`)
                        ...
                        connection.query(query, (err, result) => {
                            err ? console.log(err) : console.log(`Row inserted into wish_list_items_tracking ${user.id} ${(alias.alias_id).toString()} ${item.id} `)
                        })
                        query = `SELECT wish_list_items.priority as 'priority', items.name as 'item_name', items.id as 'item_id', items.img_uri,items.level, zones.name as 'zone_name', npcs.name as 'npc_name', npcs.id as 'npc_id', items.slot as 'item_slot', items.type as 'item_type', items.img_name as 'item_image_name', zones.id as 'zone_id' FROM wish_list_items INNER JOIN items on items.id = wish_list_items.item_id INNER JOIN zones on zones.id = items.zone_id INNER JOIN users on wish_list_items.user_id = users.id INNER JOIN npcs on npcs.id = items.npc_id WHERE users.id = ${user.id}`
                        connection.query(query, (err, rows) => {
                            if (err) {
