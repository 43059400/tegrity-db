...
module.exports = {
    ...
    upatePriorty: (user, item, alias, cb, pos) => {
        pool.getConnection((error, connection) => {
            connection.query(`SELECT * FROM wish_list_items INNER JOIN items on items.id = wish_list_items.item_id INNER JOIN users on users.id = wish_list_items.user_id WHERE users.id = '${user.id}' AND wish_list_items.alias_id = '${alias.alias_id}' AND zone_id = '${item.zone_id}'`, (error, result) => {
                result = result || []
                let newPriorityList = []
                ...
                connection.query(`INSERT INTO wish_list_items_tracking(action_id, user_id, alias_id, item_id, details, time_stamp) VALUES ('3', '${user.id}', '${alias.alias_id}', '${item.id}', '${JSON.stringify(item)}', now())`, (error, result) => {
                ...
                newPriorityList.forEach((element) => {
                    connection.query(`UPDATE wish_list_items SET priority='${element.priority}', time_stamp=Now() WHERE user_id='${user.id}' AND alias_id='${alias.alias_id}' AND item_id='${String(element.item_id)}'`, (error, result) => {
                        if (error) 
                        { console.log(error) }
                    })
                })
    ...
    insertWish: (user, item, alias, cb) => {
        pool.getConnection((error, connection) => { 
            ...
            connection.query(query, (error, result) => {
                if(result.length >= 5) {
                    console.log(`5+ wishes already for ${user.id} ${alias.alias_id} ${item.zone_id}`)
                    ...
                    if (result !== undefined) {
                        ...
                        query = `INSERT INTO wish_list_items_tracking(action_id, user_id, alias_id, item_id, details, time_stamp) VALUES (1, ${user.id}, ${alias.alias_id}, ${item.id}, '${JSON.stringify(item)}', NOW())`
                        connection.query(query, (err, result) => {
            ...
            connection.query(query, (err, row) => {
                if(row !== undefined) {
                    ...
                    connection.query(query, (err, result) => {
                        ...
                        query = `INSERT INTO wish_list_items_tracking(action_id, user_id, item_id, alias_id, details, time_stamp) VALUES (2, ${user.id}, ${alias.alias_id}, ${item.id}, '${JSON.stringify(row)}', NOW())`
                        connection.query(query, (err, result) => {
